# SQLite3 Multiple Ciphers 2.3.5 / SQLite 3.53.2 Agent Protocol

**Version:** 2.0.2
**Updated:** 2026-06-11
**Inherits:** [.codex/UNIVERSAL_ENGINEERING_CONTRACT.md](./UNIVERSAL_ENGINEERING_CONTRACT.md) v2.0.0+
**Scope:** projects that build, vendor, link, wrap, configure, distribute, test, or operate **SQLite3 Multiple Ciphers 2.3.5**, based on **SQLite 3.53.2**. Includes C and C++ integrations, amalgamation builds, static or shared library packaging, embedded applications, CLIs, services, language bindings, JNI/JNA, Python/Rust/Node/.NET/Java/Kotlin wrappers, SQL migrations, encrypted database files, PRAGMA/URI configuration, key and rekey flows, backups, WAL/journal behavior, build flags, and cross-platform distribution.
...
- Whether the secure cipher-state nullification path retained in SQLite3MC 2.3.4+ is still intact. It looks redundant; removing it is a security regression.
...
- vendored source files, especially `sqlite3mc_amalgamation.c`, `sqlite3mc_amalgamation.h`, `sqlite3ext.h`, any separately shipped `sqlite3.c` / `sqlite3.h` copies when the pinned upstream package includes them, patches, generated amalgamation scripts, and third-party manifests;
...
## 3. Baseline posture: SQLite3MC 2.3.5 and SQLite 3.53.2
...
SQLite3MC 2.3.5 includes the upstream SQLite 3.53.2 baseline and retains the secure nullification of cipher data structures on freeing that landed in 2.3.4. Treat any edit around cipher state cleanup as security-sensitive. Do not remove zeroization, nullification, or cleanup paths because they look redundant — this is exactly the kind of code where Naur's "amorphous additions" warning bites in reverse.

SQLite 3.53.2 includes the WAL-reset database corruption fix from 3.53.1 and the follow-on upstream patch line that 2.3.5 vendors. Do not downgrade to a pre-fix SQLite baseline without explicitly accepting the risk and recording the justification (per universal contract §1.5).

### 3.2 SQLite 3.53.2 feature posture

Use SQLite 3.53.2 capabilities only when the deployed runtime is guaranteed to be SQLite3MC 2.3.5 / SQLite 3.53.2 or newer.

Notable 3.53.2 behavior for agents:
...
- `json_array_insert()` and `jsonb_array_insert()` are available in the 3.53.2 baseline.
...
- New C interfaces such as `sqlite3_str_truncate()`, `sqlite3_str_free()`, `sqlite3_carray_bind_v2()`, `SQLITE_PREPARE_FROM_DDL`, `SQLITE_UTF8_ZT`, `SQLITE_LIMIT_PARSER_DEPTH`, and `SQLITE_DBCONFIG_FP_DIGITS` are available only when the runtime really is 3.53.2+.
...
SQLite 3.52.0 was withdrawn upstream. Do not select SQLite3MC 2.3.0 / SQLite 3.52.0 as a fallback baseline. If a repository already contains that version (see §0.1), surface the issue and prefer moving to SQLite3MC 2.3.5 or a project-approved fixed baseline.
...
SQLite 3.53.2 carries the upstream WAL-reset corruption fix line forward, but this does not remove the need for connection, checkpoint, and backup discipline.
...
Before using a 3.53.2 SQL feature in migrations or generated SQL, verify that all deployment targets load SQLite3MC 2.3.5 / SQLite 3.53.2 or newer.
...
SQLite 3.53.2 carries the reader-oriented CLI formatting changes introduced through QRF.
...
- binding overhead;
- query planner changes in SQLite 3.53.2.
