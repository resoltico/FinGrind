...
- Fixed release-quality timeout budgets for slow hosted runners and operator verification. The complete root gate can take about an hour before its Docker field matrix begins, and the native Windows publication proof exceeded its prior 80-minute ceiling while still making progress; their bounded CI and release-bundle budgets plus the canonical PR, merge-handoff, and tag-verifier observation window now cover the full acceptance path instead of cancelling healthy release-quality verification mid-matrix.
- Fixed staging-container release acceptance so its Docker smoke uses the same metadata-pinned Python and `uv` release-smoke environment as the other release verification jobs, rather than relying on an ambient runner installation.
...
[Unreleased]: https://github.com/resoltico/FinGrind/compare/v0.62.2...HEAD
