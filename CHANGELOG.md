...
- Fixed protected-book artifact publication on Windows when selected backup, restore, or rekey paths contain Unicode, spaces, or leading-dash names. Destination reservation now claims an absent path exclusively without treating that reservation as the staged artifact, while final pair publication remains atomic and no-clobber.
