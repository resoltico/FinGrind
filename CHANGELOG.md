...
- Fixed protected-book staging failures so native backup export opens its source book read-only and backup and restore map native export, copy, key-generation, and re-encryption errors to deterministic named `storage-runtime-failure` responses with the selected destination path instead of exposing raw storage exceptions.
