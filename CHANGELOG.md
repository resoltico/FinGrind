...
- Fixed protected attestation-key and artifact publication on Windows. FinGrind now evaluates only ACL entries effective on the checked directory, recognizes only the SID-backed LocalSystem and built-in Administrators principals as the operating-system trust boundary around an otherwise private output directory, and normalizes Win32 directory-flush and handle-close success before shared durability evaluation. Caller-selected ACLs remain validation-only and no untrusted principal is admitted.
