...
- Fixed protected-book backup publication on Windows when selected paths contain Unicode, spaces, or leading-dash names. Backup export now uses SQLite's native backup interface with an already-reserved owned stage instead of embedding a destination filename in a SQL statement, while final pair publication remains the sole atomic no-clobber claim on each selected artifact path.
...
- Fixed protected-book staging failures so native backup export opens its source book read-only, retries bounded transient SQLite backup locks on its initialized native backup handle, and maps backup and restore export, copy, key-generation, and re-encryption errors to deterministic named `storage-runtime-failure` responses with the selected destination path instead of exposing raw storage exceptions.
