...
- Fixed protected-book staging failures so backup and restore map native export, copy, key-generation, and re-encryption errors to deterministic named `storage-runtime-failure` responses with the selected destination path instead of exposing raw storage exceptions.
