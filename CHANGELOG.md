...
- Fixed protected-book artifact publication on Windows when selected backup, restore, or rekey paths contain Unicode, spaces, or leading-dash names. Destination reservation now remains a durable sidecar record, SQLite materializes its backup stage from that retained ownership proof rather than a pre-created database file, and final pair publication makes the sole atomic no-clobber claim on each selected artifact path.
