...
- Added opaque ascending keyset pagination to `account-ledger`, with explicit `--limit` and `--cursor` inputs, accepted pagination facts in `resolvedQuery`, and an optional `nextCursor` for continuing a running ledger without duplicating prior rows.
...
- Added explicit `amend-account` and `retire-account` operations with typed outcomes and lifecycle audit facts. Amendments preserve account identity and declaration time; retirement preserves the account and ledger history while retaining the historical reversal path.
- Added a narrow Latvian 2026 monthly-payroll context. It derives supported payroll components from gross EUR wages, retains immutable runs and obligation settlements with compensating reversals, and exposes a reconciliation register in text, JSON, CSV, and PDF without claiming EDS filing or worker-status determination.
...
- Hard-broke the public contract to protocol `28` and the protected-book format to `46`. Accrual cut-offs, the narrow Latvian payroll lifecycle, fixed assets, financing, and realized foreign exchange own typed request grammars, account roles, durable origins and applications, lifecycle ordering, append-only reconciliation, and reports; retained lifecycle values are now bound at the storage boundary to their immutable typed posting and foreign-exchange facts. Earlier book formats are rejected rather than upgraded in place.
- Hard-broke the machine report and failure contract at protocol `24`. Every report now returns a family-specific semantic JSON payload with canonical book identity, a non-replay `resolvedQuery`, generation metadata, enum tokens, and exact money objects, including trading income-statement gross-profit totals; CSV now exports one family-specific typed row table with paired currency and integer-minor-unit columns, while text and PDF remain human projections. Failure categories are now declared once by the published response descriptor catalog and include `internal` for FinGrind software failures, so no response code can silently receive a fallback category.
...
- Changed the protected-book maintenance contract incompatibly to protocol `22` while keeping book format `39`. Existing key-file flags are now read-only inputs; every newly generated secret uses an explicit absent-target flag: `generate-book-key-file --new-book-key-file`, `rekey-book --new-book-key-file`, `backup-book --new-backup-key-file`, and `restore-book --new-book-key-file`.
- Changed `backup-book` to generate an independent backup key instead of reusing the live-book key. `restore-book` now requires a fresh destination key and requires `--replace-existing-book` before it can replace an existing selected book; `rekey-book` always generates its new key and no longer accepts caller-supplied replacement secret material.
- Changed the published accounting-kernel scope to name the typed settled and on-credit business-event flows, inventory operations, tax selection, reversals, opening positions, owner flows, and period-close operations that are implemented today, together with explicit boundaries for credit events, tax, inventory, direct journals, and foreign exchange.
- Changed every rejected or error JSON envelope to publish one machine-readable `category` beside its top-level `code` and `message`, using the stable `structural-invalid`, `domain-semantic`, `precondition`, `unsupported-selection`, or `internal` taxonomy.
- Changed the public contract incompatibly to protocol `25` and protected-book format `40`. `declare-tax-registration` remains a pure registration operation and never creates accounts implicitly; complete setup now belongs to the atomic plan template.
- Changed Account Registry lifecycle semantics so only never-posted, unreferenced accounts can be amended, while retirement requires a zero balance and no live operational binding. Retired accounts reject new ordinary authored postings, but a true reversal of retained history remains valid even when that reversal changes the retired account's historical balance.
- Changed the public capability catalog to publish the narrow Latvian monthly-payroll profile with its exact operative boundary instead of incorrectly excluding payroll as a whole. The accounting-scope ADR, domain-model guidance, and payroll ADR now distinguish that implemented context from unsupported payroll profiles and name its typed commands and reconciliation report.
...
- Fixed JSON report query shape so omitted account-balance and account-ledger effective-date bounds remain explicitly `null` in their own `resolvedQuery` records, and clarified that `inspect-rekey-rollback` needs no passphrase source because it only discovers sibling artifact paths while restore and delete require the current source to act on a selected artifact.
...
- Fixed account-ledger continuation balances so every page includes the final row from its supplied cursor in its prior running total while retaining whole-range closing balances.
- Fixed protected-book verification after account retirement so a legally retired account with historical journal lines remains a complete book. New ordinary use remains rejected at admission and by SQLite, while reversal lineage is the only permitted inactive-account write path.
- Fixed Latvian monthly-payroll persistence in protected SQLite books. The durable posting-fact grammar and settlement validation now accept the supported payroll facts and enforce the exact payroll journal without ambiguous SQL references.
