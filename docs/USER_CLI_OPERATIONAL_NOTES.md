...
- A passphrase chosen for a new protected book must contain at least 16 Unicode characters and at least one non-whitespace character. Existing protected books retain their established passphrase compatibility; use `rekey-book` to rotate an older weak secret.
- Book key files must use POSIX owner-only permissions (`0400` or `0600`) on macOS/Linux or a Windows owner-only ACL on Windows, their containing directory must also remain owner-only, and the public quick starts keep those files under `./.local/fingrind/secrets/` instead of beside the book.
...
- `tax-obligation` aggregates retained `AppliedTax` facts from typed tax-selected events for one declared registration and its matching period. It is not a general-ledger reconciliation of the registration's payable or recoverable account, a remittance/payment ledger, or a filed return; use the ordinary accounting reports to inspect direct journal lines and account balances.
...
- `trial-balance --output text` presents balance-bearing account columns in one compact table rather than one vertical block per account. Use `list-accounts` or the JSON/CSV trial-balance output when account type, normal balance, and activation metadata are needed beside the balances.
...
- The public packaged CLI bundles its own Java 26 runtime and managed SQLite 3.53.4 / SQLite3 Multiple Ciphers 2.5.1 native library.
...
- Gradle-driven local runs, the source-checkout wrapper, and the container image use a managed SQLite 3.53.4 / SQLite3 Multiple Ciphers 2.5.1 shared library.
...
- Wrong passphrases, damaged or truncated protected books, unexpected `-wal` or `-shm` residue,
  unauthenticated appended bytes, and foreign or otherwise unauthenticatable protected SQLite
  variants are reported as the deterministic `protected-book-verification-failed` error instead of
  leaking raw SQLite symptoms such as `SQLITE_NOTADB`. The sole longer-file exception is one
  complete manifest-attested backup artifact, whose signed snapshot is verified before it is read.
  An authenticated FinGrind book whose `user_version` is non-current instead returns
  `unsupported-book-format-version` with `detectedBookFormatVersion` and
  `supportedBookFormatVersion`; it is never migrated or opened.
