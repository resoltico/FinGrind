...
Describe "MSVC setup policy adapter" {
    ...
    It "uses policy-generated cmd arguments and preserves UTF-16 environment output through the final native adapter" {
        ...
        $commandLine | Should -Be '"C:\Rīga Visual Studio\2022\BuildTools\Common7\Tools\VsDevCmd.bat" -arch=x64 -host_arch=arm64 >nul && set'
        ...
        $script:NativeProcessCalls[0].Arguments | Should -Be @("/d", "/u", "/v:off", "/s", "/c", $commandLine)
