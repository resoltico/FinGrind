...
print_usage() {
    printf '%s\n' \
        'Usage: ./scripts/verify-structural-governance.sh [--surface build-logic-kotlin] [--surface gradle-kts] [--surface json-resource] [--surface markdown-docs] [--surface shell-release] [--surface python-support] [--surface sqlite-sql]' \
        '' \
        'Verifies FinGrind structural-governance budgets for non-Java control-plane surfaces.' \
        'Without --surface, the verifier checks build-logic-kotlin, gradle-kts, json-resource, markdown-docs, shell-release, python-support, and sqlite-sql.'
}
