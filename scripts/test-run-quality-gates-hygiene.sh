...
[[ -x "${jacoco_artifacts_verifier}" ]] || die "missing executable JaCoCo artifact verifier"
...
printf '%s' "${help_output}" | grep -F './scripts/verify-jacoco-artifacts.sh' >/dev/null || die \
    "quality-gate help no longer documents JaCoCo artifact verification"
...
(( verifier_call_line < jacoco_verifier_call_line )) || die \
    "repository hygiene verification must run before the JaCoCo artifact verifier"
(( jacoco_verifier_call_line < gradle_check_line )) || die \
    "JaCoCo artifact verification must run before Gradle quality gates"
