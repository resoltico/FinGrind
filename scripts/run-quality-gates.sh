...
print_usage() {
    printf '%s\n' \
        ...
        '  2. ./scripts/verify-structural-governance.sh --surface build-logic-kotlin --surface gradle-kts --surface markdown-docs --surface python-support --surface sqlite-sql' \
        '  3. ./gradlew check coverage' \
        '  4. ./gradlew -p gradle/build-logic check' \
        '  5. ./scripts/verify-build-logic-plugin-jar.sh' \
        '' \
