...
fg_cli_wrapper_refresh_raw_jar_if_needed \
    "failed to refresh the source-checkout launcher JAR from the current checkout"
fg_cli_wrapper_verify_raw_jar \
    "missing source-checkout launcher JAR at ${fg_cli_wrapper_raw_jar}; run ./gradlew :cli:shadowJar prepareManagedSqlite" \
    "source-checkout launcher JAR at ${fg_cli_wrapper_raw_jar} is not synchronized with the current checkout; rerun ./gradlew :cli:shadowJar prepareManagedSqlite"
