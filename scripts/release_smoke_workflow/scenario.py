...
def smoke_path(work_root: Path, argument_path_mode: str, relative_path: Path) -> SmokePath:
    ...
    if argument_path_mode == ARGUMENT_PATH_MODE_ABSOLUTE:
        return SmokePath(local_path=local_path, argument=str(local_path))
    if argument_path_mode == ARGUMENT_PATH_MODE_WORK_ROOT_RELATIVE:
        return SmokePath(local_path=local_path, argument=relative_path.as_posix())
    raise ReleaseSmokeFailure(
        "unsupported release-smoke argument path mode: " + argument_path_mode
    )
