...
/** Proves inherit-only Windows ACL entries do not grant access on the directory being checked. */
class PrivateOutputDirectoryInheritOnlyAclTest {
  ...
  @Test
  void admission_ignoresInheritOnlyAclEntriesBecauseTheyGrantNothingOnTheCheckedDirectory() {
    FakeFilesystemAccess filesystem =
        PrivateOutputDirectoryTestFilesystem.fakeFilesystemAccess(OWNER);
    PrivateOutputDirectory.AclState inheritedAcl =
    ...
    for (Path path : List.of(Path.of("/"), ROOT, ANCESTOR, OUTPUT)) {
      filesystem.putAcl(path, inheritedAcl);
    }
