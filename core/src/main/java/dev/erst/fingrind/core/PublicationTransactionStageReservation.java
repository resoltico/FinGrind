 ...
 * short-lived producer destination; it must never be rendered, persisted by a caller, or reused
 * as a cleanup handle.
 */
public final class PublicationTransactionStageReservation {
  ...
  PublicationTransactionStageReservation(PublicationTransactionJournal journal) {
    ...
    for (PublicationTransactionMember member : checkedJournal.members()) {
      paths.put(member.memberId(), member.stagePath());
    }
    this.stagePaths = Map.copyOf(paths);
  }
  ...
  public Path stagePath(String memberId) {
    ...
    if (stagePath == null) {
      throw new IllegalArgumentException("Publication transaction has no member named " + memberId + ".");
    }
    return stagePath;
  }
}
