...
final class PrivateOutputDirectoryPathTopology {
  ...
  static void requireLexicalRealDirectoryPath(
    ...
    requireLexicalDirectoryComponent(root, filesystemAccess);
    Path component = root;
    for (Path name : checkedDirectory) {
      component = component.resolve(name);
      requireLexicalDirectoryComponent(component, filesystemAccess);
    }
  }

  private static void requireLexicalDirectoryComponent(
      Path component, PrivateOutputDirectory.FilesystemAccess filesystemAccess) throws IOException {
