...
/** Shared CLI invocation, credential, and workflow fixtures for tests. */
class CliWorkflowFixtureSupport extends CliWorkflowDoubleSupport {
  ...
  /**
   * Supplies the fixture founder credential and adds JSON output for an intentional protected-book
   * mutation when the caller did not select an output mode.
   *
   * <p>Tests must choose this helper deliberately: JSON output alone never implies authorization.
   */
  protected static String[] attestedJsonArguments(String... arguments) {
    return jsonArguments(attestedArguments(arguments));
  }
