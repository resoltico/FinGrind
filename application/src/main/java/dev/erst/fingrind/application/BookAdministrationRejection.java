...
public sealed interface BookAdministrationRejection
  ...
  static List<MachineContract.RejectionDescriptor> descriptors() {
    return Arrays.stream(BookAdministrationRejection.class.getPermittedSubclasses())
        .map(type -> descriptorFor(type.asSubclass(BookAdministrationRejection.class)))
        .toList();
  }

  private static MachineContract.RejectionDescriptor descriptorFor(
      Class<? extends BookAdministrationRejection> rejectionType) {
    if (rejectionType == BookAdministrationRejection.BookAlreadyInitialized.class) {
    ...
    if (rejectionType == BookAdministrationRejection.BookNotInitialized.class) {
      return new MachineContract.RejectionDescriptor(
          "book-not-initialized",
          "Command refused because the selected book does not exist or has not been initialized with open-book.");
    }
    if (rejectionType == BookAdministrationRejection.BookContainsSchema.class) {
      return new MachineContract.RejectionDescriptor(
          "book-contains-schema",
          "Book initialization refused because the selected SQLite file already contains schema objects.");
    }
    if (rejectionType == BookAdministrationRejection.NormalBalanceConflict.class) {
      return new MachineContract.RejectionDescriptor(
    ...
    throw new IllegalStateException(
        "Unsupported book-administration rejection type: " + rejectionType.getName());
  }
