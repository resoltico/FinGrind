...
final class SqliteNativeLibrary {
  ...
  private static final ReentrantLock SQLITE_API_LOCK = new ReentrantLock();
  private static final AtomicReference<SqliteApi> SQLITE_API = new AtomicReference<>();
    ...
    try (Arena arena = Arena.ofConfined()) {
      ...
      int timeoutResult =
          (int)
              sqliteApi
                  .sqlite3BusyTimeout()
                  .invokeExact(databaseHandle, SQLITE_BUSY_TIMEOUT_MILLIS);
    ...
    try (Arena arena = Arena.ofConfined()) {
      ...
      int resultCode =
          (int)
              sqliteApi
                  .sqlite3PrepareV2()
                  .invokeExact(databaseHandle, sql, -1, statementPointer, tailPointer);
      if (resultCode != SQLITE_OK) {
        throw failure(databaseHandle, resultCode, sqliteApi);
      }
      return resultCode;
  ...
  static void bindNull(MemorySegment statementHandle, int parameterIndex)
    ...
    try {
      int resultCode =
          (int) sqliteApi.sqlite3BindNull().invokeExact(statementHandle, parameterIndex);
      if (resultCode != SQLITE_OK) {
  ...
  static String columnText(MemorySegment statementHandle, int columnIndex) {
    ...
    try {
      ...
      int byteLength =
          (int) sqliteApi.sqlite3ColumnBytes().invokeExact(statementHandle, columnIndex);
  ...
  static int compareVersions(String leftVersion, String rightVersion) {
    int parts = Math.max(versionPartCount(leftVersion), versionPartCount(rightVersion));
    for (int index = 0; index < parts; index++) {
      int left = versionPart(leftVersion, index);
      int right = versionPart(rightVersion, index);
      if (left != right) {
  ...
  private static SqliteApi api() {
    SQLITE_API_LOCK.lock();
    try {
      if (SQLITE_API.get() == null) {
        SQLITE_API.set(loadApi());
      }
      return SQLITE_API.get();
    } finally {
      SQLITE_API_LOCK.unlock();
    }
  }
  ...
  static SqliteLibraryTarget configuredLibraryTarget(
    ...
    if (configuredLibraryPath != null) {
      ...
      return new SqliteLibraryTarget(
          "managed", Path.of(normalizedPath).toAbsolutePath().normalize().toString());
    }
  ...
  static void requireOpenConfigurationSuccess(MemorySegment databaseHandle, int resultCode)
      throws SqliteNativeException {
    requireOpenConfigurationSuccess(databaseHandle, resultCode, api());
  }
  ...
  private static int versionPartCount(String version) {
