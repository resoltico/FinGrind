  ...
  static ContractRequestShapes.RequestInputDescriptor requestInput() {
    return new ContractRequestShapes.RequestInputDescriptor(
        ...
        List.of(
            ...
            operation(OperationId.TRANSFER_PERIOD_RESULT),
        ...
        List.of(
            ...
            "supported report commands also accept --pdf-out <path> and write one PDF artifact on successful execution without changing the command's selected stdout output mode; successful JSON exports publish the normalized artifact path under artifacts[], while text and csv exports report the same normalized artifact path on the diagnostics stream",
